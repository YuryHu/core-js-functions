 ...
 *
 */
const displayStr = (str) => `"${str}"`;

const displayArr = (arr) => {
  const res = arr.map((el) => (typeof el === 'string' ? displayStr(el) : el));
  return `[${res.join(',')}]`;
};

function logger(func, logFunc) {
  return (...args) => {
    const args1 = args.map((el) =>
      Array.isArray(el) ? displayArr(el) : el.toString()
    );
    logFunc(`${func.name}(${args1}) starts`);
    const res = func(...args);

    logFunc(`${func.name}(${args1}) ends`);
    return res;
  };
}
